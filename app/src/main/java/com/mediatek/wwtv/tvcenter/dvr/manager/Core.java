/**
 * @Description: TODO()
 */
package com.mediatek.wwtv.tvcenter.dvr.manager;

import com.mediatek.wwtv.tvcenter.R;

/**
 *
 */
public class Core {

    public final static int ROOT_VIEW = R.id.linear_glview;

    public final static String PVR_DISK_TAG         = "/pvrdisk.tag";
    public final static String PVR_DISK             = "pvr_disk_tag";
    public final static String TSHIFT_DISK_TAG      = "/tshiftdisk.tag";
    public final static String TSHIFT_DIR_DISK       = "/tshift";
    public final static String DISK_SPEED           = "disk_speed_tag";
    public final static String NO_DEVICES           = "No Device";
    public final static float DEFAULT_DISK_SPEED    = 6.0f;
    public final static int AUTO_DISSMISS_TIMER     = 15000;

    
    @Override
    public String toString() {
        android.util.Log.d("Dvrconst","DvrconsttoString");
        return "DvrconsttoString"; 
    }
}
