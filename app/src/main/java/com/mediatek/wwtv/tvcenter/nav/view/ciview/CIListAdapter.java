...
public class CIListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "getView-->"+position);
		if (convertView == null) {
		...
		if (mData != null && mData.length > 0) {
			if (mData[position] != null && mData[position].length()> 0) {
				com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "setText:"+mData[position].trim());
	...
	public void setCIGroup(String[] data) {
		com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "setCIGroup");
		this.mData = data;
	}
