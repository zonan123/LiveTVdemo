...
public class EPGEu2ndActivity extends BaseActivity implements
	...
	private final Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case EPGConfig.EPG_UPDATE_API_EVENT_LIST:
				changeTimeViewsShow(EPGEu2ndActivity.this.dayNum, mStartHour);
				loadEventList();
				break;
...
//		mMtkTvBanner = MtkTvBanner.getInstance();
		...
		mFreeViewHDLogo.setVisibility(mDataAction.isCountryUK()?View.VISIBLE:View.INVISIBLE);
		getChannelList();
	}
			...
			public void onTurnCH(int inputDigit) {
				...
				for (int i = 0; i < channelList.size(); i++) {
					...
					if (channelInfo.getmChanelNum() == inputDigit) {
						...
						com.mediatek.wwtv.tvcenter.util.MtkLog.v(
								TAG,
								"channelInfo=[num:"
	...
	private void turnChannel(EPGChannelInfo changeChannel) {
		mEPGEuHelper.turnChannel(changeChannel);
	}
	...
	private void initUI() {
		...
		mFreeViewHDLogo = findViewById(R.id.epg_freeviewhd_logo);
  ...
  private void getChannelList() {
    ...
    com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "EPG onResume");
	  super.onResume();
	}
	...
	private Runnable mGetCurrentTimeRunnable = new Runnable() {
		...
		public void run() {
			...
			lastTime = curTime;
			String mDate = EPGUtil.formatCurrentTime(EPGEu2ndActivity.this,mDataAction.is3rdTVSource());// tmCvt.getDetailDate(DataReader.getCurrentDate());
			com.mediatek.wwtv.tvcenter.util.MtkLog.v(TAG, "mGetCurrentTimeRunnable~mDate="+mDate);
	...
	private void changeTimeViewsShow(int dayNum, int startHour) {
	  ...
	  com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "changeTimeViewsShow>>>>" + dayNum + "  " + startHour);
...
//	}
	...
	public void updateEventDetails(EPGProgramInfo programInfo) {
		...
		String title=(mCurrentSelectedProgramInfo == null || TextUtils.isEmpty(mCurrentSelectedProgramInfo.getTitle()))?this.getResources().getString(R.string.nav_epg_no_program_title):mCurrentSelectedProgramInfo.getTitle();
