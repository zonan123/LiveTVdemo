...
public class ActionAdapter extends BaseAdapter implements ScrollAdapter,
    ...
    private static final String DISEQC_12_ITEM = "DisEqC 1.2";
    ...
    public View getScrapView(ViewGroup parent) {
        ...
        return inflater.inflate(R.layout.settings_list_item, parent, false);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        /*for special view has left and right key*/
        if(action.hasLeftRight()){
        	chevronView.setImageResource(R.drawable.nav_double_arrow_channeledit);
        ...
        boolean isVisible = action.hasNext()|action.hasLeftRight();
        chevronBackgroundView.setVisibility(isVisible ? View.VISIBLE : View.INVISIBLE);
        ...
        if (action.hasMultilineDescription()) {
            ...
            description.setMaxHeight(
                    getDescriptionMaxHeight(convertView.getContext(), title));
        } else {
...
//        com.mediatek.wwtv.tvcenter.util.MtkLog.d("ActionAdapter", "onScrolled v=="+view);
        if (hasFocus) {
            if (view != null) {
    ...
    private void changeFocus(View v, boolean hasFocus, boolean shouldAnimate) {
        ...
        float chevronBgAlpha = 0;
        if(action.hasLeftRight() || action.hasNext()){
        	 chevronBgAlpha= 1;
        }
        ...
        setAlpha(chevronBackground, shouldAnimate, chevronBgAlpha);
...
//                        });
   ...
   /* private void fadeIn(View v) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_DPAD_LEFT:
            	if(event.getAction() == 0){
            		boolean cando = action.isEnabled() && action.hasLeftRight();
            		com.mediatek.wwtv.tvcenter.util.MtkLog.d("ActionAdapter", "press left-right key can change value:"+cando);
	               	if(cando){
	               		changeValueForLeftRight(keyCode,action,v);
	               		handled = true;
	               	}else{
	               		handled = false;
	               	}
    ...
    private void changeValueForLeftRight(int keycode,Action action,View mSelView){
        action.mPrevInitValue = action.mInitValue;
        if(keycode == KeyEvent.KEYCODE_DPAD_RIGHT){//right
            if(action.mInitValue < action.mOptionValue.length-1){
                action.mInitValue ++;
            }else{
                action.mInitValue = 0;
            }
            action.setDescription(action.mInitValue);
        }else{//left
            if(action.mInitValue <= 0){
                action.mInitValue = action.mOptionValue.length-1;
            }else{
                action.mInitValue --;
            }
            action.setDescription(action.mInitValue);
        ...
        //refresh text partly
        TextView description = (TextView) mSelView.findViewById(R.id.action_description);
        description.setText(action.getDescription());
        com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "motor callback ...itemId:"+action.mItemID+",description:"+action.getDescription());
        if (action.mItemID.equals(MenuConfigManager.DVBS_DETAIL_DISEQC_MOTOR)) {
            Action disEqcE=(Action)getItem(0);
            Action disEqcT=(Action)getItem(1);
            if(action.getDescription().equalsIgnoreCase(DISEQC_12_ITEM)){
                disEqcE.setDescription("Disable");
                disEqcT.setDescription("Disable");
                notifyDataSetChanged();
                com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "motor  notifyDataSetChanged...");
            }
        }
    ...
    private void fadeCheckmarks(final View v, final Action action, int duration, int delay,
        ...
        if (actionCheckSetId != Action.NO_CHECK_SET) {
            ...
            for (int i = 0, size = mActions.size(); i < size; i++) {
                ...
                if (!a.equals(action) && a.getCheckSetId() == actionCheckSetId && a.isChecked()) {
                    a.setChecked(false);
     ...
     */
    private static synchronized Integer getDescriptionMaxHeight(Context context, TextView title) {
