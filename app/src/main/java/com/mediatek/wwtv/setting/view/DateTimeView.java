...
//	private final Context mContext;
    ...
    protected void onDraw(Canvas canvas) {
		...
		if (mDate != null) {
			...
			// y = mHeight / 2;
			y = (mHeight + mPaint.measureText(mDateChars, 0, 1) * 1.2f) / 1.5f;
			...
			if(mCurrentSelectedPosition>=mDate.length()){
				mCurrentSelectedPosition=mDate.length()-1;
			}
	...
	public void input(char ch, DataItem mDataItem) {
		...
		if (validate(tempStr, mCurrentSelectedPosition, ch, mType)) {
			...
			if (mType == DATETYPE) {
                if (mCurrentSelectedPosition == 0 || mCurrentSelectedPosition == 1 && Integer
                        .valueOf(mDate.substring(0, 2)) >= 21
                        || mCurrentSelectedPosition == 2 && Integer.valueOf(mDate.substring(2, 4)) > 0
						|| mCurrentSelectedPosition == 3
								&& mDateChars[2] == '0' && mDateChars[mCurrentSelectedPosition] > '0'
                        || ((mCurrentSelectedPosition == 2 || mCurrentSelectedPosition == 3
...
//			}
	...
	private void changeNotValidate() {
		String tempStr = String.valueOf(mDateChars);
		if (mType == DATETYPE) {
            if (mCurrentSelectedPosition == 0 || mCurrentSelectedPosition == 1
                    || mCurrentSelectedPosition == 2 || mCurrentSelectedPosition == 3) {
				if (Integer.valueOf(tempStr.substring(0, 4)) > 2100) {
					mDateChars[0] = '2';
					mDateChars[1] = '1';
					mDateChars[2] = '0';
					mDateChars[3] = '0';
