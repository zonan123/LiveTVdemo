...
public class SetupUtils {
    ...
    protected SetupUtils(Context context) {
        ...
        mTunerInputId = TvContract.buildInputId(new ComponentName(context,
            TvInputService.class));
    }
    ...
    public void onTvInputSetupFinished(
        ...
        onSetupDone(inputId);
        final TIFChannelManager manager =
            ...
            manager.addListener(
                    new TIFChannelManager.Listener() {
                        ...
                        public void onLoadFinished() {
                            ...
                            updateChannelsAfterSetup(inputId, postRunnable);
                        }
    ...
    private static void updateChannelsAfterSetup(
            final String inputId, final Runnable postRunnable) {
        final TIFChannelManager manager =
			TvSingletons.getSingletons().getChannelDataManager();
                    ...
                    for (TIFChannelInfo channel : manager.getChannelList()) {
                        if (channel.mInputServiceName.equals(inputId)) {
                            if (!channel.mIsBrowsable) {
                                manager.updateBrowsable(channel.mId, true, true);
    ...
    public void markNewChannelsBrowsable() {
        ...
        TvInputManagerHelper tvInputManagerHelper = getTvInputManagerHelper(mContext);
        ...
        if (!newInputsWithChannels.isEmpty()) {
            for (TIFChannelInfo channel : channelDataManager.getChannelList()) {
                if (newInputsWithChannels.contains(channel.mInputServiceName)) {
                    channelDataManager.updateBrowsable(channel.mId, true);
                }
            }
    ...
    public boolean isSetupDone(String inputId) {
        ...
        com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "isSetupDone: (input=" + inputId + ", result= " + done + ")");
        return done;
    }
        ...
        if (!removedInputList.isEmpty()) {
            ...
            for (String input : removedInputList) {
                try {
                    ...
                    com.mediatek.wwtv.tvcenter.util.MtkLog.i(TAG, "TV input (" + input + ") is removed but package is not deleted");
                } catch (NameNotFoundException e) {
                    com.mediatek.wwtv.tvcenter.util.MtkLog.i(TAG, "TV input (" + input + ") and its package are removed");
...
//        SoftPreconditions.checkState(inputId != null);
        com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "onSetupDone: input=" + inputId);
        if (!mRecognizedInputs.contains(inputId)) {
            com.mediatek.wwtv.tvcenter.util.MtkLog.i(TAG, "An unrecognized input's setup has been done. inputId=" + inputId);
            mRecognizedInputs.add(inputId);
            mSharedPreferences
                    .edit()
                    .putStringSet(PREF_KEY_RECOGNIZED_INPUTS, mRecognizedInputs)
        ...
        if (!mKnownInputs.contains(inputId)) {
            com.mediatek.wwtv.tvcenter.util.MtkLog.i(TAG, "An unknown input's setup has been done. inputId=" + inputId);
            mKnownInputs.add(inputId);
