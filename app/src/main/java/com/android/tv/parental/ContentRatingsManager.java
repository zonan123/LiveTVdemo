...
public class ContentRatingsManager {
    ...
    public void updateForChangeLanguage() {
        mContentRatingSystems.clear();
        ContentRatingsParser parser = new ContentRatingsParser(mContext);
        parser.clearRatingSystem();
        List<TvContentRatingSystemInfo> infos = mTvInputManager.getTvContentRatingSystemList();
        if (infos != null) {
            for (TvContentRatingSystemInfo info : infos) {
                List<ContentRatingSystem> list = parser.parse(info);
            ...
            Log.d(TAG, "updateForChangeLanguage: TvContentRatingSystemInfo = null");
        }
    }
    
    /** Returns the content rating system by the give country. */
    @Nullable
    public ContentRatingSystem getContentRatingSystemByCountry(String country) {
    	com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "mContentRatingSystems="+mContentRatingSystems+",country="+country);
        for (ContentRatingSystem s : mContentRatingSystems) {
    ...
    public ContentRatingSystem getContentRatingSystem(String contentRatingSystemId) {
    	com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "mContentRatingSystems="+mContentRatingSystems+",contentRatingSystemId="+contentRatingSystemId);
        for (ContentRatingSystem ratingSystem : mContentRatingSystems) {
        	com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "atingSystem.getId().getId()="+ratingSystem.getId());
