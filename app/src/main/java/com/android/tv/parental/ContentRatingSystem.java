...
public class ContentRatingSystem {
	public static final String TAG = "ContentRatingSystem";
        ...
        public ContentRatingSystem build() {
            ...
            if (mCountries != null) {
                if (mCountries.size() == 1) {
                    ...
                    Log.d(TAG, "MtkTvConfig.getInstance().getCountry() " + MtkTvConfig.getInstance().getCountry());
                    String currentCountry = Utils.convertConurty(MtkTvConfig.getInstance().getCountry());
                    Log.d(TAG, "mCountries = " + mCountries.toString());
                    Log.d(TAG, "currentCountry = " + currentCountry);
                    if (mCountries.contains(currentCountry)) {
                    	Log.d(TAG, "mCountries.contains");
                        ...
                        if(CommonIntegration.isUSRegion() && "US".equals(locale.getCountry())
                                && mCountries.contains("CA")){
                            int index = mCountries.indexOf("CA");
                            sb.append(new Locale("", mCountries.get(index)).getDisplayCountry());
                        }else{
                        	Log.d(TAG, "getDisplayCountry = " + new Locale("", currentCountry).getDisplayCountry());
                        	sb.append(new Locale("", currentCountry).getDisplayCountry());
                        }
                    } else {
                    	Log.d(TAG, "other countries");
    ...
    public static class Rating implements Comparable<Rating>{
        ...
        public int compareTo(Rating rating){
        	if(this.mContentAgeHint-rating.mContentAgeHint>0){
        		return -1;
        	}else if(this.mContentAgeHint-rating.mContentAgeHint==0){
        		return 0;
        	}
        	else{
        		return 1;
        ...
        public static class Builder {
            ...
            private Rating build(List<SubRating> allDefinedSubRatings) {
                ...
                if (allDefinedSubRatings == null && !mSubRatingNames.isEmpty()) {
                ...
                return new Rating(mName, mTitle, mValue, mDescription, mIcon, mContentAgeHint, subRatings);
            }
        }
    }
    ...
    public static class SubRating {
        ...
        public boolean equals(Object sub){
            if(mName == null || sub == null
                    || !(sub instanceof SubRating)){
                return false;
            }
