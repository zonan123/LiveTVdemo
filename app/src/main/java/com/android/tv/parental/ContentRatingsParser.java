...
public class ContentRatingsParser {
    ...
    public List<ContentRatingSystem> parse(TvContentRatingSystemInfo info) {
        ...
        if (DEBUG) {
        	Log.d(TAG, "Parsing rating system for " + uri);
        }
        try {
            String packageName = uri.getAuthority();
            String curPackageName=mContext.getPackageName();
            com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, " packageName="+packageName +",curPackageName="+curPackageName);
            if(TextUtils.equals(packageName,curPackageName) && mRatingSystems != null){
                com.mediatek.wwtv.tvcenter.util.MtkLog.d(TAG, "return instance mRatingSystems");
